...
namespace EvaEngine {
	...
	public:
		// シーンの追加
		template<class T>
		static void AddScene(const std::u8string& sceneName) {
		...
		// シーンのロード
		static void LoadScene(const std::u8string& sceneName);
		static void LoadScene(const UINT& sceneID);

		// 現在のシーン名を取得
		static std::u8string GetCurrentSceneName();
		// 前のシーン名をを取得
		static std::u8string GetPreviousSceneName();
	};
}